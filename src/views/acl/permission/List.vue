    ...
    */    
    removePermission(permission) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        const result = await this.$APIs.permission.removePermission(permission.id)
    ...
    addOrUpdatePermission() {
      this.$refs.permission.validate(async valid => {
        if (valid) {
          ...
          const result = await this.$APIs.permission[perm.id ? 'updatePermission' : 'addPermission'](perm)
