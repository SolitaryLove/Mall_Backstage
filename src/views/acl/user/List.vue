...
export default {
  ...
  methods: {
    ...
    */
    async getRoles () {
      const result = await this.$APIs.user.getRoles(this.user.id)
    ...
    */
    async assignRole () {
      ...
      this.loading = true
      const result = await this.$APIs.user.assignRoles(userId, roleIds)
      this.loading = false
    ...
    */
    revomveUsers () {
      this.$confirm('确定删除吗?').then(async () => {
        await this.$APIs.user.removeUsers(this.selectedIds)
    ...
    */
    async removeUser (id) {
      await this.$APIs.user.removeById(id)
      this.$message.success('删除成功')
    ...
    */
    async getUsers (page=1) {
      ...
      const result = await this.$APIs.user.getPageList(page, limit, searchObj)
    ...
    addOrUpdate () {
      this.$refs.userForm.validate(valid => {
        if (valid) {
          const {user} = this
          this.loading = true
          this.$APIs.user[user.id ? 'update' : 'add'](user).then((result) => {
            this.loading = false
